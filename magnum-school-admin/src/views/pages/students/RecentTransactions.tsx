      ...
      <ul className="flex-1 overflow-y-auto min-h-[120px]">
        {paginatedTransactions.length === 0 ? (
          ...
          paginatedTransactions.map((tx) => {
            // Support both legacy and backend transaction types
            const isBackend = 'reference_number' in tx;
            const key = isBackend ? tx.reference_number : tx.id;
            const type = isBackend
              ? tx.transaction_type === 'credit'
                ? 'recharge'
                : 'withdraw'
              : tx.type;
            const label = isBackend ? tx.description : tx.label;
            const dateObj = isBackend ? new Date(tx.date) : undefined;
            const dateStr =
              isBackend && dateObj && !isNaN(dateObj as any)
                ? format(dateObj, 'PPP')
                : isBackend
                  ? tx.date
                  : tx.date;
