...
const buildTarget = (endpoint: string) => {
  ...
  // If backend base already contains /api, avoid duplicating
...
export const POST = async (request: NextRequest) => {
  try {
    ...
    const target = buildTarget('login');
    ...
    // If backend requires OTP, do not set session cookie - just relay the response
    if (resp.status === 202 || data?.requires_otp) {
      return NextResponse.json(data || { message: 'Accepted' }, {
        status: resp.status,
      });
    }
    ...
    if (resp.ok && data) {
      ...
      // Set NextAuth session cookie so client-side `getSession()` works
      if (encoded) {
    ...
    // Otherwise, relay backend response/status
